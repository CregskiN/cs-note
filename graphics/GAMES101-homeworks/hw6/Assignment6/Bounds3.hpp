...
class Bounds3 {
   public:
    ...
    Bounds3() {
        ...
        pMax = Vector3f(minNum, minNum, minNum);  // pMax pMin 确定一个包围盒
...
inline bool Bounds3::IntersectP(const Ray& ray, const Vector3f& invDir,
    ...
    // 若取轴负向为增，则交换tmax tmin
    if (dirIsNeg[0] > 0) std::swap(tMax_x, tMin_x);
    if (dirIsNeg[1] > 0) std::swap(tMax_y, tMin_y);
    if (dirIsNeg[2] > 0) std::swap(tMax_z, tMin_z);
