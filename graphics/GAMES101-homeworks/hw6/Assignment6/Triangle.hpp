...
inline Intersection Triangle::getIntersection(Ray ray) {
    ...
    // 光线从物体外侧往物体内侧打，点乘<0 则可以点亮
    if (dotProduct(ray.direction, normal) > 0)
        return inter;
    double u, v, t_tmp = 0;
    Vector3f pvec = crossProduct(ray.direction, e2);  // S1 = D cross E2
    double det = dotProduct(e1, pvec);                // 分子 E1 dot S1
    // 浮点数绝对值 < epsilon // 有啥用？
    if (fabs(det) < EPSILON)
        return inter;

    double det_inv = 1. / det;
    Vector3f tvec = ray.origin - v0;       // S = O - v0
    u = dotProduct(tvec, pvec) * det_inv;  // b1 = S dot S1 * 1/det
    if (u < 0 || u > 1)                    // 若不满足重心坐标，则不相交
        return inter;
    Vector3f qvec = crossProduct(tvec, e1);         // S2 = S cross E1
    v = dotProduct(ray.direction, qvec) * det_inv;  // b2 = D corss S2 * 1/det
    if (v < 0 || u + v > 1)                         // 若不满足重心坐标，则不相交
        return inter;
    t_tmp = dotProduct(e2, qvec) * det_inv;  // t = E2 cross S2 * 1/det

    // TODO find ray triangle intersection
    if (t_tmp < 0)  // 若物体表面在光源后面，则照不到 不相交
        return inter;
    inter.happened = true;
