...
struct Shader : public IShader {
    ...
    mat<4, 3, float> varying_clipped_positions;  // vertex_positions of clipped coordinates (not ndc)
    mat<3, 3, float> varying_normals;            // vertex_normals of clipped coordinates (not ndc)
    mat<3, 3, float> ndc_clipped_positions;      // vertex_positions of clipped coordinates (ndc)
    ...
    virtual Vec4f vertex(int iface, int nthvert) {
        ...
        varying_clipped_positions.set_col(nthvert, gl_Vertex);
        ndc_clipped_positions.set_col(nthvert, proj<3>(gl_Vertex / gl_Vertex[3])); // projection division
        return gl_Vertex;
    }
    ...
    virtual bool fragment(Vec3f bar, TGAColor& color) {
        ...
        mat<3, 3, float> A;
        A[0] = ndc_clipped_positions.col(1) - ndc_clipped_positions.col(0);
        A[1] = ndc_clipped_positions.col(2) - ndc_clipped_positions.col(0);
        A[2] = bn;
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < model->nfaces(); ++i) {
        ...
        triangle(shader.varying_clipped_positions, shader, image, z_buffer);
    }
